...
class Solution {
    public String[] getFolderNames(String[] names) {
        ...
        for (String name : names) {
            if (!map.containsKey(name)) {
                ...
                String checkName = name + "(" + count + ")";
                while (map.containsKey(checkName)) {
                    count++;
                    checkName = name + "(" + count + ")";
                }
                map.put(name, count);
                map.put(checkName, 1);
                result[pos++] = checkName;
            }
        }
        return result;
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Arrays.toString(new Solution().getFolderNames(new String[]{"kaido", "kaido(1)", "kaido", "kaido(1)", "kaido(2)"})));
    }
}
